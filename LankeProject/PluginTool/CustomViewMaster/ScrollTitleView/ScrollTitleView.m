...
@implementation ScrollTitleView
{
    ...
    CGFloat _textWidth;
    ZHTTextScrollMode _currentScrollModel;
    ZHTTextScrollMoveDirection _currentMoveDirection;
...
-(void)startScroll{
    ...
    switch (_currentScrollModel) {
        ...
        case ZHTTextScrollContinuous:
        {
            if (_textWidth > self.frame.size.width) {
                if (_currentMoveDirection == ZHTTextScrollMoveLeft) {
        ...
        case ZHTTextScrollIntermittent:
        {
            if (_currentMoveDirection == ZHTTextScrollMoveLeft) {
                [self creatLabel1WithFrame:CGRectMake(0, 0, _textWidth, self.frame.size.height)];
            }else{
...
-(void)creatLabel1WithFrame:(CGRect)frame{
    _contentLabel1 = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
...
- (void)contentMove {
    ...
    if (_textWidth < self.frame.size.width) {//如果字符串长度小于控件宽度，不滚动
        ...
        if (_currentScrollModel == ZHTTextScrollWandering) {//往返模式除外
            [self moveWandering];
        }
        return;
    }
