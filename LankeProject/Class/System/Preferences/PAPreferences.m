...
- (NSString *)configureDefaultsKeyForPropertyName:(NSString *)key {
    
    if ([self respondsToSelector:@selector(defaultsKeyForPropertyName:)]) {
        return [self performSelector:@selector(defaultsKeyForPropertyName:) withObject:key];
    }
    return key;
}
    ...
    if (self = [super init]) {
        ...
        if ([self respondsToSelector:@selector(setupDefaults)]) {
            ...
            for (NSString *key in defaults) {
                ...
                NSString *transformedKey = [self configureDefaultsKeyForPropertyName:key];
            ...
            if (isDynamic) {
                if (shouldRetain) {
                    ...
                    if ([self isValidType:type]) {
                        NSString *defaultsKey = [self configureDefaultsKeyForPropertyName:name];
 ...
 *
 *  @return https://developer.apple.com/library/ios/documentation/Cocoa/Reference/Foundation/Classes/NSUserDefaults_Class/#//apple_ref/occ/instm/NSUserDefaults/initWithSuiteName:
 *  @ Use Scenarios:
 *     * 当开发一个suite app并且需要在多个app之间进行传递数据的时候
 *     * 当开发的是一个extension app需要在多个app之间或者其包含的app之间传递数据的时候
 *  @ Warning: 一般情况的开发下不需要进行这个设置
 */
- (NSString *) setupSuiteName{
