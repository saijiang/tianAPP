...
- (void) configCellWithData:(id)data
{
    self.type=YES;
    self.data=data;
    self.modBtn.hidden = YES;
    self.deleteBtn.hidden = YES;
    
    self.userName.text=data[@"userName"];
    self.phoneLB.text=data[@"mobileNum"];
    self.addressLB.text=[NSString stringWithFormat:@"%@%@%@%@%@%@号%@室",data[@"provinceName"],data[@"cityName"],data[@"countyName"],data[@"districtAddress"],data[@"districtName"],data[@"buildingNum"],data[@"roomNum"]];
...
-(void)setDefaultPropery
{
    [UserServices
     setDefaultProperyWithUserId:[KeychainManager readUserId]
     ...
     completionBlock:^(int result, id responseObject)
     {
         if (result==0)
         {
             ...
             [self sendObject:@"setDefaultPropery"];
         }
     }];
}
...
-(void)loadCellWithDataSource:(id)dataSource
{
    ...
    self.userName.text=dataSource[@"receiveName"];
    self.phoneLB.text=dataSource[@"receivePhone"];
    self.addressLB.text=[NSString stringWithFormat:@"%@ %@ %@ %@%@",dataSource[@"province"],dataSource[@"city"],dataSource[@"county"],dataSource[@"areaInfo"],dataSource[@"detailedAddress"]];
...
{
    [UserServices
     setDefaultAddressWithuserId:[KeychainManager readUserId]
...
{
    [UserServices
     deleteAddressWithuserId:[KeychainManager readUserId]
     addressId:self.data[@"id"]
     completionBlock:^(int result, id responseObject) {
         if (result==0)
         {
