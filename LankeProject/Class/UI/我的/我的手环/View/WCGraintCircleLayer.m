...
-(instancetype)initGraintCircleWithBounds:(CGRect)bounds Position:(CGPoint)position FromColor:(UIColor *)fromColor ToColor:(UIColor *)toColor LineWidth:(CGFloat) linewidth ProgressWith:(float)progress
{
    if (self = [super init]) {
        if (progress>1.0) {
            progress=1.0;
        ...
        for (int i = 0; i < colors.count -1; i++) {
            ...
            shapelayer.strokeEnd = progress+0.015;
