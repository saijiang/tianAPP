...
@interface MyGoodsOrderViewController ()<EHHorizontalSelectionViewProtocol,UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource>
...
-(void)initiateNetworkListRequest{
    ...
    [UserServices
     getGoodsOrderListWithuserId:[KeychainManager readUserId]
     pageSize:pageSize
     pageIndex:pageIndex
     ...
     completionBlock:^(int result, id responseObject)
    {
        if (result == 0) {
            ...
            for (NSDictionary * data_ in data) {
                GoodsOrderStatusViewModel * viewModel = [GoodsOrderStatusViewModel viewModelWithData:data_];
...
- (void) requestOrderHandleWith:(id)object orderInfo:(id)orderInfo{
    ...
    {
        [UserServices
         getGoodsOrderDetailWithOrderCode:orderInfo[@"orderCode"]
         completionBlock:^(int result, id responseObject)
         {
             if (result==0)
             {
