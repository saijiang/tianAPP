...
@interface JDShopOrderViewController ()<EHHorizontalSelectionViewProtocol,UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource>
...
@property (nonatomic ,strong) PersonCenterOrderStateView * orderStateView;

@end

@implementation JDShopOrderViewController
...
- (void)createUI{
    ...
    LKWeakSelf;
//    self.orderStateView = [[PersonCenterOrderStateView alloc] init];
//    self.orderStateView.frame = CGRectMake(0, 0, DEF_SCREEN_WIDTH, 200);
//        self.orderStateView.stateName = @[@"全部",@"代付款",@"待收货",@"已完成",@"已取消",@"测试1",@"测试2",@"测试3",@"测试4",@"测试5",@"测试6"];
//    self.orderStateView.maxDisplayCount = 3;
//    self.orderStateView.buttonWidth = 80;
//    self.orderStateView.stateBlock = ^(NSInteger index ){
//        LKStrongSelf
//        if (index) {
//            _self.orderState = [NSString stringWithFormat:@"0%lu",index + 1];
//        }else{
//            _self.orderState = @"01";
//        }
//        [_self refresh];
...
//    [self addSubview:self.orderStateView];
    ...
    self.orderSegments = @[@"全部",@"待付款",@"待收货",@"已完成",@"已取消"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    GoodsOrderStatusViewModel * viewModel = self.responseDatas[indexPath.section];
    [cell configJDShopCellWithData:viewModel];
    LKWeakSelf
...
-(void)initiateNetworkListRequest{
    ...
    [UserServices
     getJDOrderListWithUserId:[KeychainManager readUserId]
     ...
     completionBlock:^(int result, id responseObject)
     {
         if (result == 0) {
             ...
             for (NSDictionary * data_ in data) {
                 GoodsOrderStatusViewModel * viewModel = [GoodsOrderStatusViewModel JDShopviewModelWithData:data_];
...
- (void) requestOrderHandleWith:(id)object orderInfo:(id)orderInfo{
    ...
    if ([object isEqualToString:@"确认收货"])
    {
        DQAlertViewController * alert = [[DQAlertViewController alloc] initWithTitle:@"提示" message:@"确定收货？" buttons:@[@"取消",@"确定"] afterDismiss:^(NSInteger buttonIndex) {
            ...
            if (buttonIndex)
            {
                ...
                [UserServices
                 changeGroupOrderStatusWithjdOrderId:orderInfo[@"jdOrderId"]completionBlock:^(int result, id responseObject)
                 {
                     if (result == 0)
                     {
                         //刷新我的订单列表
  ...
  else  if ([object isEqualToString:@"取消订单"])
    {
        DQAlertViewController * alert = [[DQAlertViewController alloc] initWithTitle:@"提示" message:@"确定取消订单？" buttons:@[@"取消",@"确定"] afterDismiss:^(NSInteger buttonIndex) {
            ...
            if (buttonIndex)
            {
                ...
                [UserServices cancelJDOrderWithuserId:[KeychainManager readUserId] jdOrderId:orderInfo[@"jdOrderId"] completionBlock:^(int result, id responseObject) {
                    if (result == 0)
                    {
                        [self refresh];
                    }
    ...
    else if ([object isEqualToString:@"删除订单"])
    {
        DQAlertViewController * alert = [[DQAlertViewController alloc] initWithTitle:@"提示" message:@"确定删除订单？" buttons:@[@"取消",@"确定"] afterDismiss:^(NSInteger buttonIndex) {
            ...
            if (buttonIndex)
            {
                ...
                [UserServices deleteJDOrderWithuserId:[KeychainManager readUserId] jdOrderId:orderInfo[@"jdOrderId"]  completionBlock:^(int result, id responseObject) {
                    if (result == 0)
                    {
                        [self refresh];
                    }
    ...
    else if ([object isEqualToString:@"去付款"])
    {
        ...
        pay.orderType = @"07";
        pay.orderCode = orderInfo[@"jdOrderId"];
        //pay.sumPrice = [orderInfo[@"zkOrderPrice"] floatValue];
...
//         }];
    ...
    {
        [UserServices getJDOrderDetailWithjdOrderId:orderInfo[@"jdOrderId"]
                                    completionBlock:^(int result, id responseObject)
        {
            if (result==0)
            {
...
//         }];
    ...
    else if ([object isEqualToString:@"查看物流"])
    {
        ...
        logistic.orderCode=orderInfo[@"jdOrderId"];
