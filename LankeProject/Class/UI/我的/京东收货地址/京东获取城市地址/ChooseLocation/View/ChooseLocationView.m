...
@property (nonatomic,strong) NSMutableArray * townDataSouce;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.townDataSouce=[NSMutableArray array];
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([self.tableViews indexOfObject:tableView] == 0){
        ...
        [self.townDataSouce removeAllObjects];

        //1.1 获取下一级别的数据源(市级别,如果是直辖市时,下级则为区级别)
        AddressItem * provinceItem = self.dataSouce[indexPath.row];
        self.cityDataSouce = [[CitiesDataTool sharedManager] queryAllRecordWithSheng:provinceItem.name];
        if(self.cityDataSouce.count == 0){
            
            if (self.tableViews.count!=1&&self.tableViews.count>0) {
                [self removeLastItem:self.tableViews.count-1];
                
            }
            if (provinceItem.name.length!=0) {
                
                self.shenStr=provinceItem.name;
            }
...
//        AddressItem * item = self.dataSouce[indexPath.row];
        if (item.name.length!=0) {
            self.shenStr=item.name;
        }
        
    }else if ([self.tableViews indexOfObject:tableView] == 1){
        
        //AddressItem * item1 = self.dataSouce[indexPath.row];
        [self.townDataSouce removeAllObjects];
        [self.districtDataSouce removeAllObjects];
        AddressItem * cityItem = self.cityDataSouce[indexPath.row];
        self.districtDataSouce = [[CitiesDataTool sharedManager]queryAllRecordWithSheng:self.shenStr shi:cityItem.name];
        NSIndexPath * indexPath0 = [tableView indexPathForSelectedRow];
...
//            }
            ...
            if (self.tableViews.count>2) {
                [self removeLastItem:self.tableViews.count-2];
            ...
            if (cityItem.name.length!=0) {
                self.shiStr=cityItem.name;
            }
             return indexPath;
       ...
       AddressItem * item = self.cityDataSouce[indexPath.row];
        ...
        [self.townDataSouce removeAllObjects];
        AddressItem * shiItem = self.districtDataSouce[indexPath.row];
        self.townDataSouce = [[CitiesDataTool sharedManager]queryAllRecordWithSheng:self.shenStr shi:self.shiStr qu:shiItem.name];
        NSIndexPath * indexPath0 = [tableView indexPathForSelectedRow];
        
        if ([indexPath0 compare:indexPath] != NSOrderedSame && indexPath0) {
            if (self.tableViews.count>3) {
                [self removeLastItem:self.tableViews.count-3];
                
            }
//            for (int i = 0; i < self.tableViews.count - 1; i++) {
//                [self removeLastItem];
//            }
            if (self.townDataSouce.count==0) {
             
                [self setUpAddress:shiItem.name];
                
            }else{
                
                [self addTopBarItem];
                [self addTableView];
                [self scrollToNextItem:shiItem.name];
        ...
        AddressItem * item = self.townDataSouce[indexPath.row];
        if (self.townDataSouce.count==0) {
            [self setUpAddress:@"无"];
...
- (void)setUpAddress:(NSString *)address{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        for (int i=0; i<self.topTabbarItems.count; i++) {
            UIButton*itemBtn=self.topTabbarItems[i];
            if (i==0) {
                for (AddressItem*item in self.dataSouce) {
                    if ([itemBtn.currentTitle isEqualToString:item.name]) {
            ...
            if (i==1) {
                for (AddressItem*item in self.cityDataSouce) {
                    if ([itemBtn.currentTitle isEqualToString:item.name]) {
                        [self.itemArray insertObject:item atIndex:1];
                    }
                }
            }
            if (i==2) {
                for (AddressItem*item in self.districtDataSouce) {
                    if ([itemBtn.currentTitle isEqualToString:item.name]) {
                        [self.itemArray insertObject:item atIndex:2];
                    }
                }
            }
            if (i==3) {
                
                for (AddressItem*item in self.townDataSouce) {
                    if ([itemBtn.currentTitle isEqualToString:item.name]) {
                        [self.itemArray insertObject:item atIndex:3];
                    }
                }
            }
        }
...
- (void)removeLastItem:(NSInteger)num{
...
- (void)setAreaCode:(NSString *)areaCode{
    ...
    NSString*countyName=@"";
    NSString*townName=@"";
    NSArray*nameArray=[self.address componentsSeparatedByString:@" "];
    if (nameArray.count>3) {
        provinceName=nameArray[0];
        cityName=nameArray[1];
        countyName=nameArray[2];
        townName=nameArray[3];
    }else{
    ...
    self.shenStr=provinceName;
    self.shiStr=cityName;

    self.dataSouce=[[CitiesDataTool sharedManager]queryAllProvince];
    self.cityDataSouce = [[CitiesDataTool sharedManager] queryAllRecordWithSheng:provinceName];
    
    self.districtDataSouce = [[CitiesDataTool sharedManager]queryAllRecordWithSheng:provinceName shi:cityName];
    
      self.townDataSouce = [[CitiesDataTool sharedManager]queryAllRecordWithSheng:provinceName shi:cityName qu:countyName];
    //2.1 添加市级别,地区级别列表
    [self addTableView];
    [self addTableView];
    //2.3 添加底部对应按钮
    ...
    UIButton * countyBtn = self.topTabbarItems[2];
    [countyBtn setTitle:countyName forState:UIControlStateNormal];
    ...
    if (self.townDataSouce.count!=0) {
        ...
        [self.topTabbarItems makeObjectsPerformSelector:@selector(sizeToFit)];
        [_topTabbar layoutIfNeeded];
        [self changeUnderLineFrame:countyBtn];
   ...
   //2.4 设置偏移量
    ...
    [self setSelectedProvince:provinceName andCity:cityName andDistrict:countyName andTown:townName];
}

//初始化选中状态
- (void)setSelectedProvince:(NSString *)provinceName andCity:(NSString *)cityName andDistrict:(NSString *)districtName andTown:(NSString *)townName{
    ...
    for (int i = 0; i <self.townDataSouce.count; i++) {
        AddressItem * item = self.townDataSouce[i];
