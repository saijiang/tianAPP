...
@interface ModyPasswordViewController ()
{
    id msgId;
}
...
@implementation ModyPasswordViewController
...
- (void) finishModifyPasswordHandle
{
    ...
    {
        [UnityLHClass showHUDWithStringAndTime:@"两次密码不一致，请重新输入"];

    }
    else
    {
        [UserServices
         updateWalletPasswordWithuserId:[KeychainManager readUserId]
         msgId:msgId
         authCode:self.codeTextField.text
         newPassword:self.passwordTetField.text
