...
@interface DishesDetailViewController ()<UITableViewDataSource,UITableViewDelegate>
...
- (void) creatUI{
    ...
    //
    _dishesDetailTableView = [[UITableView alloc] initWithFrame:CGRectZero style:UITableViewStyleGrouped];
    ...
    _headerView.bGradeButtonHandle = ^(){
        ...
        [LKUserAccessManager userNextStepJudgeAccessLoginHandle:nil normalHandle:^{
            ...
            gradContentView.bCommitButtonHandle = ^(NSString * content ,NSInteger value,PopupContentView *popupContentView){
                LKStrongSelf
                [_self requestCommitGradeWithContent:content rate:value withPopupView:popupContentView];
            };
...
- (void) requestCommitGradeWithContent:(NSString *)content rate:(NSInteger)rate withPopupView:(GradePopupContentView *)popupView{
    
    NSString * type = @"01";
    NSString * dishesId = [NSString stringWithFormat:@"%@",self.dishesData[@"dishesId"]];
    NSString * dishesName = [NSString stringWithFormat:@"%@",self.dishesData[@"dishesFoodName"]];
    NSString * scores = [NSString stringWithFormat:@"%ld",(long)rate];
    NSString * userId = [KeychainManager readUserId];
    NSString * userName = [KeychainManager readUserName];
    
    [UserServices saveEvaluationWithRestaurantType:type dishesId:dishesId dishesName:dishesName evalScores:scores evalContent:content userId:userId userName:userName completionBlock:^(int result, id responseObject) {
       
        if (result == 0) {
            [UnityLHClass showHUDWithStringAndTime:@"评分成功"];
