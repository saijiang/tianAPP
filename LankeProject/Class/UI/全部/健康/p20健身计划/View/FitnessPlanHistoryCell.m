...
@interface FitnessPlanHistoryCell()
...
@implementation FitnessPlanHistoryCell
+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    FitnessPlanHistoryCell *cell=[tableView dequeueReusableCellWithIdentifier:@"FitnessPlanHistoryCell"];
    if (!cell)
    {
        cell=[[FitnessPlanHistoryCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"FitnessPlanHistoryCell"];
    }
    return cell;
...
-(void)createCell
{
    ...
    headerBaseImage.userInteractionEnabled=YES;
    ...
    self.name=[UnityLHClass masonryLabel:@"健身计划A" font:28.0 color:BM_WHITE];
    ...
    self.time=[UnityLHClass masonryLabel:@"2016.11.1-2016.12.31" font:16.0 color:BM_WHITE];
    ...
    self.status=[UnityLHClass masonryLabel:@"已结束" font:13.0 color:[UIColor colorWithRed:0.93 green:0.86 blue:0.40 alpha:1.00]];
...
//    UILabel *scan =[UnityLHClass masonryLabel:@"查看报告" font:16.0 color:BM_WHITE];
     self.scan=[UnityLHClass masonryButton:@"查看报告" font:16.0 color:BM_WHITE];
    [headerBaseImage addSubview:self.scan];
    [self.scan mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(headerBaseImage.mas_centerX);
        make.bottom.mas_equalTo(headerBaseImage.mas_bottom).offset(-10);
        
    }];
    self.scan.bottomlineWithColor=BM_WHITE;
...
-(void)loadCellWithDataSource:(id)dataSource
{
    ...
    self.time.text=[NSString stringWithFormat:@"%@-%@",dataSource[@"cycleStartDate"],dataSource[@"cycleEndDate"]];
    ...
    //cycleFlg 	String 	周期标识，01：周期未开始 02：周期未结束 03：周期已完成
    switch ([dataSource[@"cycleFlg"] integerValue])
    {
        case 1:
        {
