...
- (void)prepareSectionLayoutForSections:(NSIndexSet *)sectionIndexes
{
    ...
    [sectionIndexes enumerateIndexesUsingBlock:^(NSUInteger section, BOOL *stop) {
        ...
        // todo:
        //        columnMinY += ((section == 0  && needsToPopulateSignHeaderAttributes)? self.signHeaderHeight : 0);// some bug here
...
- (CGFloat)stackedSectionHeightUpToSection:(NSInteger)upToSection
{
    ...
    // todo:
    BOOL needsToPopulateSignHeaderAttributes = NO;//(self.signHeaderAttributes.count != 0);
    
    CGFloat stackedSectionHeight = needsToPopulateSignHeaderAttributes ? self.signHeaderHeight : 0.0;
