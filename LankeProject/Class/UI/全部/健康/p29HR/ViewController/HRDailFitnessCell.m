...
@implementation HRDailFitnessCell


+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    HRDailFitnessCell *cell=[tableView dequeueReusableCellWithIdentifier:@"HRDailFitnessCell"];
    if (!cell)
    {
        cell=[[HRDailFitnessCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"HRDailFitnessCell"];
    }
    return cell;
...
-(void)createCell
{
    ...
    CGFloat gwith=DEF_SCREEN_WIDTH/3;
...
-(void)loadCellWithDataSource:(id)dataSource
{
    ...
    self.lable3.text=[NSString stringWithFormat:@"%@",dataSource[@"stepNum"]];
}
